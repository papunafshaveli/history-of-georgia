...
The dev loop has **two parts**: a native dev client APK (installed once on the emulator/device) and the Metro bundler (started fresh each session for JS hot-reload).

**First-time setup (or after any native-module change in `package.json` / `app.config.ts` plugins):**

```
npx expo run:android   # ~5–10 min: compiles native debug, installs to emulator, launches app, starts Metro
npx expo run:ios       # same, for iOS simulator
```

**Every subsequent dev session** — the dev client is already installed, so you only need Metro:

```
npm start              # alias for `expo start`; press `a` for Android, `i` for iOS once Metro prints the menu
```

Or skip the menu and auto-launch the app:
...
JS-only edits (most of your work) hot-reload through Metro instantly. You only need to re-run `expo run:android|ios` when you add a native dependency, edit `app.config.ts` plugins, or change `runtimeVersion`. The generated `android/` and `ios/` folders are gitignored — they're regenerated by `expo prebuild` on every native build.

## Start Metro only (legacy alias)

```
npx expo start --dev-client --clear
```

The `--clear` flag wipes the Metro bundler cache — useful when JS changes don't seem to be picked up after a `package.json` edit.
