...
- **Latest production OTAs (runtime 2.0.0):**
  - **Android** `019e08e8-d366-7879-8ded-ba846842e9f7` (group `9edaecc3-f3d4-4584-88d5-a35834ef7d96`) — leaderboard podium polish: crown icon next to the rank number when the user is on the podium (replaces the bottom "შენ" text label so cards stay symmetric), `adjustsFontSizeToFit` on `podiumName` so longer Georgian display names ("პაპუნა ფშაველი") don't ellipse, side-card `height` retained so all 3 cards line up. Bottom tab `tabBarLabel` fontSize 14→11 so "სტატისტიკა" no longer truncates. Predecessor `019e0806-9e70-74f8-8d6b-094718119ca1` (group `9866437d-2379-4a89-9da2-5cb4ddf2eb9f`) — UI polish: tab bar safe-area inset (no overlap with system 3-button nav), restart button `paddingHorizontal` so Georgian script doesn't touch the border, leaderboard `refresh()` on auth state change so weekly entries appear immediately. Initial post-SHA-fix OTA was `019e0797-...`.
