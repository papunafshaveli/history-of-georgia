...
- **iOS v2.0.0 LIVE on App Store** (Apple approved 2026-05-09). Apple Sign-In + sign-out verified on real iPhone post-approval. Both auth providers now reachable on both platforms.
- **Latest production OTAs (runtime 2.0.0):**
  - **iOS** `019e0b44-30cd-7da3-868f-1a460f6d4755` (group `48713553-99cc-40f3-94de-62700cea2549`) and **Android** `019e0b46-96c4-7b72-bb52-85ae44407d84` (group `9211951b-c94c-4382-b769-8ea2d4746e0c`) — both built from `e5afddc` working tree (uncommitted as of OTA push). Leaderboard podium card `height` bumped (170→190 side, 200→220 center) so points have breathing room above the bottom border. iOS-only tab bar base height 80→55 (gated by `IS_IOS` const in `TabNavigation.tsx`) since iOS doesn't need to clear a 3-button system nav like Android.
  - Predecessor pair (commit `e5afddc`): Android `019e08e8-d366-7879-8ded-ba846842e9f7` (group `9edaecc3-f3d4-4584-88d5-a35834ef7d96`) and iOS `019e0b30-728c-77d0-9764-3dca6f215161` (group `54d56c21-428b-4c5d-9189-1bfeb7991ed9`) — leaderboard podium polish: crown icon next to the rank number when the user is on the podium (replaces the bottom "შენ" text label so cards stay symmetric), `adjustsFontSizeToFit` on `podiumName` so longer Georgian display names ("პაპუნა ფშაველი") don't ellipse, side-card `height` retained so all 3 cards line up. Bottom tab `tabBarLabel` fontSize 14→11 so "სტატისტიკა" no longer truncates.
  - Earlier Android OTA `019e0806-9e70-74f8-8d6b-094718119ca1` (group `9866437d-2379-4a89-9da2-5cb4ddf2eb9f`) — UI polish: tab bar safe-area inset (no overlap with system 3-button nav), restart button `paddingHorizontal` so Georgian script doesn't touch the border, leaderboard `refresh()` on auth state change so weekly entries appear immediately. Initial post-SHA-fix Android OTA was `019e0797-...`. Earlier iOS OTA `019e06ab-5579-7434-b6c3-4c5b5281b8f2` restored Apple + Google sign-in buttons in the anon-gate via `IS_IOS`.
